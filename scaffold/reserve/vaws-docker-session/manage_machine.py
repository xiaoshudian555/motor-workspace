...
IMAGE_RESOLVER_USER_AGENT = "mws-machine-management/1.0"
DEFAULT_WORKDIR = "/mnt/motor-workspace"
...
SENTINEL = "__MWS_JSON__="
PROGRESS_SENTINEL = "__MWS_PROGRESS__="
DEFAULT_PASSWORD_ENV = "MWS_SSH_PASSWORD"
...
def run_with_askpass(
    ...
    *,
    ...
    env_name = validate_env_name(env_name)
    temp_dir = pathlib.Path(tempfile.mkdtemp(prefix="mws-askpass-"))
    try:
...
def format_progress_event(
    ...
    return f"[mws{host}][{phase}] {message}"
...
safe_source /etc/profile.d/mws-ascend-env.sh
safe_source /usr/local/Ascend/ascend-toolkit/set_env.sh
safe_source /usr/local/Ascend/ascend-toolkit/latest/set_env.sh
safe_source /usr/local/Ascend/nnal/atb/set_env.sh "--cxx_abi=${MWS_ATB_CXX_ABI:-1}"
...
result: dict[str, object] = {
    ...
    "npu_probe": {
        ...
        "env": {
            ...
            "MWS_NPU_SOC": os.environ.get("MWS_NPU_SOC"),
        },
    },
    "image": {
...
soc_from_env = os.environ.get("SOC_VERSION") or os.environ.get("MWS_NPU_SOC")
if soc_from_env:
...
prepared_image_tag() {
  ...
  printf 'mws-session-prepared:%s-ssh-v2\n' "$image_hash"
}
...
write_host_state() {
  ...
  host_soc="$3"
  host_image="$4"
  install -d -m 0755 /etc/mws /etc/profile.d
...
if [ -n "$host_machine_type" ]; then
  export MWS_MACHINE_TYPE="$host_machine_type"
fi
if [ -n "$host_container_type" ]; then
  export MWS_CONTAINER_TYPE="$host_container_type"
fi
if [ -n "$host_soc" ]; then
  export MWS_NPU_SOC="$host_soc"
  export SOC_VERSION="$host_soc"
fi
if [ -n "$host_image" ]; then
  export MWS_CONTAINER_IMAGE="$host_image"
fi
EOF_HOST_ENV
...
set -euo pipefail
state_file=/tmp/mws-package-bootstrap.json
rm -f "$state_file"
...
PY
  ...
  for _d in $(ls -1d /usr/local/python*/bin 2>/dev/null | sort -V -r); do
    if [ -x "$_d/python3" ]; then
      _mws_runtime_python="$_d/python3"
      break
    fi
  done
  if [ -z "$_mws_runtime_python" ] && command -v python3 >/dev/null 2>&1; then
    _mws_runtime_python="$(command -v python3)"
  fi
  if [ -n "$_mws_runtime_python" ]; then
...
EOF_PIP
    ...
    echo "[mws-bootstrap] pip configured: /etc/pip.conf (index=huaweicloud)"
  else
    echo "[mws-bootstrap] runtime python not found, skipping pip setup"
  fi
INNER_PKG
}
...
install -d -m 0755 /run/sshd /etc/profile.d /etc/mws
_mws_python_dir=""
for _d in $(ls -1d /usr/local/python*/bin 2>/dev/null | sort -V -r); do
  if [ -x "$_d/python3" ]; then
    _mws_python_dir="$_d"
...
if [ -n "$_mws_python_dir" ]; then
  _path_prefix="$_mws_python_dir:/usr/local/bin:/usr/local/sbin"
else
  _path_prefix="/usr/local/bin:/usr/local/sbin"
fi
_mws_atb_cxx_abi="1"
if [ -n "$_mws_python_dir" ] && [ -x "$_mws_python_dir/python3" ]; then
  _torch_cxx_abi="$("$_mws_python_dir/python3" - <<'PY' 2>/dev/null || true
import torch
...
PY
  ...
  case "$_torch_cxx_abi" in
    0|1)
      _mws_atb_cxx_abi="$_torch_cxx_abi"
      ;;
  esac
fi
patch_atb_set_env_source() {
  file="$1"
  [ -f "$file" ] || return 0
  if [ ! -f "${file}.mws-atb-abi.bak" ]; then
    cp -a "$file" "${file}.mws-atb-abi.bak" 2>/dev/null || true
  fi
  tmp_file="$(mktemp)"
  if awk -v abi="$_mws_atb_cxx_abi" '
...
cat > /etc/profile.d/mws-ascend-env.sh <<EOF_CONTAINER_ENV
#!/bin/sh
export MWS_ATB_CXX_ABI="\${MWS_ATB_CXX_ABI:-$_mws_atb_cxx_abi}"
if [ -z "\${ASCEND_HOME_PATH:-}" ]; then
  if [ -d /usr/local/Ascend/ascend-toolkit/latest ]; then
    export ASCEND_HOME_PATH=/usr/local/Ascend/ascend-toolkit/latest
  elif [ -d /usr/local/Ascend/ascend-toolkit ]; then
...
if [ -f /usr/local/Ascend/nnal/atb/set_env.sh ]; then
  . /usr/local/Ascend/nnal/atb/set_env.sh "--cxx_abi=\${MWS_ATB_CXX_ABI:-1}" >/dev/null 2>&1 || true
fi
export LD_LIBRARY_PATH="/usr/local/Ascend/driver/lib64/common:/usr/local/Ascend/driver/lib64/driver:/usr/local/Ascend/driver/lib64:\${LD_LIBRARY_PATH:-}"
export PATH="$_path_prefix:\${PATH:-}"
if [ -n "$machine_type" ]; then
  export MWS_MACHINE_TYPE="$machine_type"
fi
if [ -n "$container_type" ]; then
  export MWS_CONTAINER_TYPE="$container_type"
fi
if [ -n "$soc" ]; then
  export MWS_NPU_SOC="$soc"
  export SOC_VERSION="$soc"
fi
if [ -n "$image" ]; then
  export MWS_CONTAINER_IMAGE="$image"
fi
EOF_CONTAINER_ENV
chmod 0644 /etc/profile.d/mws-ascend-env.sh
python3 - "$machine_type" "$container_type" "$soc" "$image" "$workdir" "$namespace" "$_mws_atb_cxx_abi" > /etc/mws/container-info.json <<'PY'
import json
import sys
...
print(json.dumps({
    ...
    "env_file": "/etc/profile.d/mws-ascend-env.sh",
}, ensure_ascii=False, indent=2))
PY
chmod 0644 /etc/mws/container-info.json
_mws_runtime_python=""
if [ -n "$_mws_python_dir" ] && [ -x "$_mws_python_dir/python3" ]; then
  _mws_runtime_python="$_mws_python_dir/python3"
...
cat > /etc/ssh/sshd_mws_config <<EOF_SSH
Port ${port}
...
PidFile /run/sshd_mws.pid
EOF_SSH
/usr/sbin/sshd -t -f /etc/ssh/sshd_mws_config
if [ -f /run/sshd_mws.pid ]; then
  kill "$(cat /run/sshd_mws.pid)" 2>/dev/null || true
  rm -f /run/sshd_mws.pid
fi
pkill -f '/etc/ssh/sshd_mws_config' 2>/dev/null || true
/usr/sbin/sshd -f /etc/ssh/sshd_mws_config
...
host_env_file="/etc/profile.d/mws-ascend-env.sh"
host_metadata_file="/etc/mws/host-info.json"
container_env_file="/etc/profile.d/mws-ascend-env.sh"
container_metadata_file="/etc/mws/container-info.json"
...
docker_pull_policy="${MWS_DOCKER_PULL_POLICY:-missing}"
...
if docker inspect "$container" >/dev/null 2>&1; then
  ...
  current_base_image=$(docker inspect -f '{{ index .Config.Labels "com.mws.base_image" }}' "$container" 2>/dev/null || true)
...
if [ "$container_exists" = "true" ]; then
  ...
  existing_base_image=$(docker inspect -f '{{ index .Config.Labels "com.mws.base_image" }}' "$container" 2>/dev/null || true)
  ...
  if [ "$use_prepared_image_cache" = "true" ] && [ -n "$existing_base_image" ]; then
    ...
    prepared_image="$(docker inspect -f '{{ index .Config.Labels "com.mws.prepared_image" }}' "$container" 2>/dev/null || true)"
  ...
  docker run --name "$container" -it -d --network host --shm-size=500g \
    --privileged=true \
    --label com.mws.managed=true \
    --label com.mws.container_ssh_port="$port" \
    --label com.mws.workdir="$workdir" \
    --label com.mws.namespace="$namespace" \
    --label com.mws.machine_type="$machine_type" \
    --label com.mws.container_type="$machine_type" \
    --label com.mws.soc="$soc" \
    -w "$workdir" \
    ...
    "${mount_args[@]}" \
    --label com.mws.base_image="$selected_image" \
    --label com.mws.run_image="$run_image" \
    --label com.mws.prepared_image="$prepared_image" \
    "$run_image" >/dev/null
...
if ! docker exec "$container" bash -lc 'command -v sshd >/dev/null 2>&1 && command -v ssh >/dev/null 2>&1'; then
  ...
  package_bootstrap="$(docker exec "$container" bash -lc 'cat /tmp/mws-package-bootstrap.json 2>/dev/null || true')"
  docker exec "$container" bash -lc 'rm -f /tmp/mws-package-bootstrap.json' >/dev/null 2>&1 || true
...
safe_source /usr/local/Ascend/ascend-toolkit/latest/set_env.sh
safe_source /usr/local/Ascend/nnal/atb/set_env.sh "--cxx_abi=${MWS_ATB_CXX_ABI:-1}"
safe_source /mnt/motor-workspace/vllm-ascend/vllm_ascend/_cann_ops_custom/vendors/vllm-ascend/bin/set_env.bash
...
def build_parser() -> argparse.ArgumentParser:
    ...
    probe.add_argument(
        "--managed-prefix",
        default="mws-",
        help="managed container name prefix (default: mws-)",
    )
    probe.add_argument(
        "--machine-type",
    ...
    mesh_export.add_argument(
        "--comment",
        required=True,
        help="stable mesh key comment, for example mws-mesh:173.125.1.2",
    )
    ...
    mesh_remove.add_argument(
        "--peer-comment",
        required=True,
        help="stable mesh comment, for example mws-mesh:173.125.1.2",
    )
    mesh_remove.add_argument(
