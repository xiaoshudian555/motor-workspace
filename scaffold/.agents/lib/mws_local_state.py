...
# --- VAWS machine-profile helpers (machine-management / session-management) ---

def _validate_machine_profile(profile: Any, *, where: str = "profile") -> dict[str, Any]:
...
def load_machine_profile(path: Path = PROFILE_PATH) -> dict[str, Any] | None:
    path = path.expanduser().resolve()
    if not path.exists():
        return None
    data = _load_json(path)
    return _validate_machine_profile(data, where=str(path))


def save_machine_profile(profile: dict[str, Any], path: Path = PROFILE_PATH) -> dict[str, Any]:
    path = path.expanduser().resolve()
    normalized = _validate_machine_profile(profile, where="profile")
...
def ensure_machine_profile(
    *,
    ...
    path = path.expanduser().resolve()
    existing = load_machine_profile(path)
    if existing is not None:
        ...
        return save_machine_profile(updated, path=path), "updated"
    ...
    return save_machine_profile(profile, path=path), "created"


# VAWS machine-management compatibility aliases.
ensure_profile = ensure_machine_profile
