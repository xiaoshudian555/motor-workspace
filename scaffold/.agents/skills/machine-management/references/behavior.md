...
- wrapper scripts stream phase progress on `stderr` as `__MWS_PROGRESS__=<json>` and keep one final JSON result on `stdout`
...
- migrate to `.motor-workspace-local/machine-inventory.json` on the next successful inventory write
...
- derive new container names from the namespace, for example `mws-alice123`
...
- never persist the password into tracked files or `.motor-workspace-local/`
...
- `/etc/profile.d/mws-ascend-env.sh`: adds the runtime Python directory and Ascend driver libs to `PATH` and `LD_LIBRARY_PATH`.
...
- persist the result as `MWS_ATB_CXX_ABI` in `/etc/profile.d/mws-ascend-env.sh` and `/etc/mws/container-info.json`
- source `/usr/local/Ascend/nnal/atb/set_env.sh` with `--cxx_abi=<0|1>` from generated MWS env scripts, smoke paths, and common image startup files
- patch existing image files such as `/etc/profile` and `/root/.bashrc` when they source ATB without an explicit ABI, keeping a `.mws-atb-abi.bak` backup

Do not rely on ATB's default dynamic ABI detection in shell startup paths. In vLLM-Ascend images, that detection can run `python3 -c "import torch"` after MWS has prepended the runtime Python to `PATH`, making simple SSH/login-shell commands take 10+ seconds.
...
Use stable key comments such as `mws-mesh:<alias-or-ip>` so later cleanup is deterministic.
