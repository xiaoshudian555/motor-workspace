...
- the skill uses `.motor-workspace-local/` as the canonical local runtime-state directory
...
- wrappers stream phase progress on `stderr` as `__MWS_PROGRESS__=<json>` while keeping the final JSON result on `stdout`
...
- the skill reuses or creates `.motor-workspace-local/machine-profile.json`
...
- the managed container uses host networking, required devices, required Ascend mounts, and `/mnt/motor-workspace` as the workdir
...
- verify / smoke prepend `driver/lib64/common`, `driver/lib64/driver`, and `driver/lib64`, and source `/etc/profile.d/mws-ascend-env.sh` when it exists
- container bootstrap records `MWS_ATB_CXX_ABI`, sources ATB with `--cxx_abi=<0|1>`, and patches common image startup files so `bash -lc true` does not spend 10+ seconds importing `torch` for ATB ABI detection
...
- inspect the generated `/etc/mws/host-info.json`, `/etc/mws/container-info.json`, and `/etc/profile.d/mws-ascend-env.sh` on a real machine after any bootstrap logic change
