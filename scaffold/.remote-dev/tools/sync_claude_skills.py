...
CLAUDE_SKILL_ROOTS = (
    ...
    (ROOT.parent / ".claude" / "skills", "scaffold/.agents/skills"),
)
MAX_SHIM_LINES = 60
...
def expected_skill_body(skill_dir: Path, canonical_root: str) -> str:
    ...
    return f"""<!-- Generated Claude Code shim from {canonical_root}/{skill_dir.name}/SKILL.md. Do not edit. -->
---
name: {name}
...
`{canonical_root}/{skill_dir.name}/SKILL.md`
...
def check_shims() -> list[str]:
    ...
    for claude_skills, canonical_root in CLAUDE_SKILL_ROOTS:
        location = claude_skills.relative_to(ROOT.parent)
        ...
        for missing in sorted(expected_names - observed_names):
            errors.append(f"missing Claude skill shim in {location}: {missing}")
        for extra in sorted(observed_names - expected_names):
            errors.append(f"extra Claude skill shim in {location}: {extra}")
        for skill_dir in source_skill_dirs():
            target = claude_skills / skill_dir.name / "SKILL.md"
            if not target.exists():
                continue
            expected = expected_skill_body(skill_dir, canonical_root)
            observed = target.read_text(encoding="utf-8")
            if observed != expected:
                errors.append(f"stale Claude skill shim in {location}: {skill_dir.name}")
            if len(observed.splitlines()) > MAX_SHIM_LINES:
                errors.append(f"Claude skill shim is too large in {location}: {skill_dir.name}")
    return errors
...
def sync_shims() -> None:
    for claude_skills, canonical_root in CLAUDE_SKILL_ROOTS:
        ...
        for skill_dir in source_skill_dirs():
            ...
            target.write_text(expected_skill_body(skill_dir, canonical_root), encoding="utf-8")
