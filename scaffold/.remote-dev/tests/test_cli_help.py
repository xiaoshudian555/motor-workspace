...
REMOTE_DEV = SCAFFOLD_SCAFFOLD_ROOT / ".remote-dev"
...
class CliHelpTests(unittest.TestCase):
    def test_cli_wrappers_have_help(self) -> None:
        scripts = sorted((SCAFFOLD_SCAFFOLD_ROOT / ".remote-dev" / "tools").glob("remote_*.py"))
        expected_scripts = {SCAFFOLD_ROOT / ".remote-dev" / "tools" / (name.replace(".", "_") + ".py") for name in TOOL_SCHEMAS}
    ...
    def test_claude_skill_shim_check_passes(self) -> None:
        proc = subprocess.run(
            [sys.executable, str(SCAFFOLD_SCAFFOLD_ROOT / ".remote-dev" / "tools" / "sync_claude_skills.py"), "--check"],
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    def test_cli_errors_return_result_contract_without_traceback(self) -> None:
        proc = subprocess.run(
            [
                sys.executable,
                str(SCAFFOLD_SCAFFOLD_ROOT / ".remote-dev" / "tools" / "remote_job_status.py"),
                "--job-id",
                "job-does-not-exist",
            ],
            capture_output=True,
