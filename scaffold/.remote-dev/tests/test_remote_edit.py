...
class RemoteEditTests(unittest.TestCase):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                def fake_run_remote_python(_endpoint, _script, payload, **_kwargs):
                    ...
                    return {
                        "status": "edited",
                        "file": {"path": "/mnt/motor-workspace/foo.py", "sha256": "after", "size": 3, "mtime_ns": 2},
                ...
                payload = file_ops.remote_edit(endpoint, file_path="/mnt/motor-workspace/foo.py", old_string="a", new_string="b")
    ...
    def test_remote_edit_with_ledger_returns_changed_file(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                state_store.substrate_root = lambda: Path(tmp)  # type: ignore[assignment]
                state_store.write_read_ledger(endpoint, {"path": "/mnt/motor-workspace/foo.py", "sha256": "before", "size": 3, "mtime_ns": 1})
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                state_store.write_read_ledger(
                    endpoint,
                    {"path": "/mnt/motor-workspace/foo.py", "sha256": "before", "size": 3, "mtime_ns": 1},
                ...
                payload = file_ops.remote_edit(
                    endpoint,
                    file_path="/mnt/motor-workspace/foo.py",
                    old_string="a",
                    new_string="b",
